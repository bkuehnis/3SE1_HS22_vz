...
public class Dashboard {
	...
	static void dashboard2(String gender) {
		long anzahl = Arrays.stream(heros)
				.filter(x -> x.getGender().equals(gender)).count();
		System.out.println(anzahl);
	}

	static void dashboard3(String eyeColor) {
		Arrays.stream(heros)
				.filter(x -> x.getGender().equals("Female") && x.getEye().equals(eyeColor))
				.forEach(x -> System.out.println(x.getName()));
	}

	static List<Hero> dashboard4(String firstChars) {
		Arrays.stream(heros)
				.filter(x -> x.getName().charAt(0) == firstChars.charAt(0))
