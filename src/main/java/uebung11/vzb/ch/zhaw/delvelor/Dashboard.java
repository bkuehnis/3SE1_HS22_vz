package uebung11.vzb.ch.zhaw.delvelor;
...
public class Dashboard {
	...
	static void dashboard3(String eyeColor) {
		Arrays.stream(heros).filter(x -> x.getGender().equals("Female")).filter(x -> x.getEye().equals(eyeColor))
				.map(x -> x.getName()).forEach(System.out::println);
		System.out.println("TODO");
	}

	static List<Hero> dashboard4(String firstChars) {
		Arrays.stream(heros).filter(x -> x.getName().startsWith(firstChars))
