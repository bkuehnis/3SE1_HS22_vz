...
public class HeroFinder{
	...
	public static void main(String[] args) {
		...
		// TODO
		List<Hero> herofilter = Stream.of(heros).filter(x -> x.getGender().equalsIgnoreCase(gender))
				.filter(a -> a.getAlignment().equalsIgnoreCase(alignment))
				.filter(c -> c.getHair().equalsIgnoreCase(hairColor))
				.filter(e -> e.getEye().equalsIgnoreCase(eyeColor))
				.collect(Collectors.toList());
		herofilter.stream().forEach(hf -> System.out.println(hf.getPublisher()+": "+hf.getName()));
		}
