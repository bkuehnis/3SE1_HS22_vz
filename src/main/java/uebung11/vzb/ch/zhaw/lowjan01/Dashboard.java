package uebung11.vzb.ch.zhaw.lowjan01;
...
public class Dashboard {
	...
	static void dashboard3(String eyeColor) {
		...
		Stream.of(heros).filter(x -> x.getGender().equals("Female")).filter(x -> x.getEye().equals(eyeColor))
				.forEach(x -> System.out.println(x.getName()));

	}

	static List<Hero> dashboard4(String firstChars) {
//		Stream.of(heros).filter(x -> x.getName().startsWith(firstChars)).sorted().forEach(x -> System.out.println(x.getName()+" "+x.getHeight()));
		/// .sorted() möchte nicht funktionieren 
		Stream.of(heros).filter(x -> x.getName().startsWith(firstChars)).forEach(x -> System.out.println(x.getName()+" "+x.getHeight()));
		return null;
	}
}
