...
public class Dashboard {
	...
	public static void main(String[] args) {
		...
		List<Hero> h = dashboard4d(keyScan.nextLine());
		if (h != null) {
	...
	static void dashboard3(String eyeColor) {
		Stream.of(heros).filter(x -> x.getGender().equals("Female") && x.getEye().equals(eyeColor))
			.forEach(x -> System.out.println(x.getName()));
	}
	
	static List<Hero> dashboard4(String firstChars) {
		return Stream.of(heros).filter(x -> x.getName().startsWith(firstChars))
				.collect(Collectors.toList());
	}
	
	static List<Hero> dashboard4d(String firstChars) {
		return Stream.of(heros).filter(x -> x.getName().startsWith(firstChars))
