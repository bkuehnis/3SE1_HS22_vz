...
public class HeroFinder {
	...
	public static void main(String[] args) {
		...
		Arrays.stream(heros).filter(x -> x.getGender().equalsIgnoreCase(gender))
				.filter(x -> x.getAlignment().equalsIgnoreCase(alignment))
				.filter(x -> x.getHair().equalsIgnoreCase(hairColor)).filter(x -> x.getEye().equalsIgnoreCase(eyeColor))
