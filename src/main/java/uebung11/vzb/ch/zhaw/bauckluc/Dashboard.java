package uebung11.vzb.ch.zhaw.bauckluc;
...
public class Dashboard {
	...
	static void dashboard2(String gender) {
		long count = Arrays.stream(heros).filter(x -> x.getGender().equalsIgnoreCase(gender)).count();
		System.out.println(count);
	}

	static void dashboard3(String eyeColor) {
		Arrays.stream(heros).filter(x -> x.getGender().equalsIgnoreCase("Female") && x.getEye().equals(eyeColor))
