...
public class Dashboard {
	...
	static void dashboard2(String gender) {
		// Stream.of(hero).filter(GENDER).count();
		//(x -> x.getGender() == gender)
		// heros.stream().filter(x -> x.getGender() == gender).count();
		// long anzahl = Arrays.stream(heros).filter(x -> x.getGender() == gender).count();
		// long anzahl = heros.stream().filter(x -> x.getGender() == gender).count();
		// long anzahl = Stream.of(heros).count();
		// long anzahl = Stream.of(heros).filter(x -> x.getHeight()>10).count();
		long anzahl = Stream.of(heros).filter(x -> x.getGender().equals(gender)).count();
