package uebung11.vza.ch.zhaw.murbalio;
...
public class Dashboard {
	...
	static void dashboard2(String gender) {
		Arrays.stream(heros);
		long anz = Stream.of(heros).filter(x -> x.getGender().equals(gender)).count();
	...
	static List<Hero> dashboard4(String firstChars) {
		Arrays.stream(heros);
		return Stream.of(heros).filter(x->x.getName().startsWith(firstChars)).sorted(Comparator.comparing(Hero::getHeight)).collect(Collectors.toList());
	}
}
