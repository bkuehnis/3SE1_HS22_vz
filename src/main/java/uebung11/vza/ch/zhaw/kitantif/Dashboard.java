package uebung11.vza.ch.zhaw.kitantif;
...
public class Dashboard {
	...
	static void dashboard2(String gender) {
		long anzahl = Stream.of(heros).filter(x ->
		x.getGender().equals(gender)).count();
		System.out.println(anzahl);
	}
	
	static void dashboard3(String eyeColor) {
		Stream.of(heros).filter(x -> 
		x.getGender().equals("Female")&& x.getEye().equals(eyeColor))
