...
public class HeroFinder {
	...
	public static void main(String[] args) {
		...
		// TODO
		Arrays.stream(heros).filter(x -> x.getGender().equalsIgnoreCase(gender) && x.getAlignment().equalsIgnoreCase(alignment)
				&& x.getHair().equalsIgnoreCase(hairColor) && x.getEye().equalsIgnoreCase(eyeColor)).forEach(x -> System.out.println(x.getPublisher() + ": " + x.getName()));
