package uebung11.vza.ch.zhaw.zejnutri;
...
public class Dashboard {
	...
	static void dashboard3(String eyeColor) {
		Arrays.stream(heros).filter(x -> x.getGender().equals("Female") && x.getEye().equals(eyeColor)).map(x -> x.getName()).forEach
		(System.out::println);		//-> checks ned ganz, de david fröge.
		
	}
	
	static List<Hero> dashboard4(String firstChars) {
		return Arrays.stream(heros).filter(x -> x.getName().startsWith(firstChars)).sorted(Comparator.comparing(Hero::getHeight)).collect(Collectors.toList());
	}
}
