package uebung11.vza.ch.zhaw.ahmaddav;
...
public class Dashboard {
	...
	static void dashboard2(String gender) {
		...
		long anzahl = Arrays.stream(heros).filter(x -> x.getGender().equals(gender)).count();
	...
	static List<Hero> dashboard4(String firstChars) {
		...
		return Arrays.stream(heros).filter(x -> x.getName().startsWith(firstChars)).collect(Collectors.toList());
	}

	static List<Hero> dashboard4V2(String firstChars) {

		return Arrays.stream(heros).filter(x -> x.getName().startsWith(firstChars))
				.sorted(Comparator.comparing(Hero::getHeight)).collect(Collectors.toList());
	}
