...
public class HeroFinder {
	...
	public static void main(String[] args) {
		...
		// TODO
		Stream.of(heros).filter(x->x.getGender().contains(gender))
		.filter(x->x.getAlignment().contains(alignment))
		.filter(x->x.getHair().contains(hairColor))
		.filter(x->x.getEye().contains(eyeColor))
		.forEach(x->System.out.println(x.getPublisher()+": "+x.getName()));
