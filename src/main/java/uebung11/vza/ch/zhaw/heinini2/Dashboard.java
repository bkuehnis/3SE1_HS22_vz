package uebung11.vza.ch.zhaw.heinini2;
...
public class Dashboard {
	...
	static void dashboard2(String gender) {
		...
		long anzahl = Stream.of(heros).filter(x->x.getGender().contains(gender)).count();
		System.out.println(anzahl);
	}
	
	static void dashboard3(String eyeColor) {
		// TODO
		Stream.of(heros).filter(x->x.getEye().equals(eyeColor)&&x.getGender().contains("Female")).forEach( x-> System.out.println(x.getName()));
		
	}
	
	static List<Hero> dashboard4(String firstChars) {
		List<Hero> h1 = Stream.of(heros).filter(x->x.getName().startsWith(firstChars))
