package uebung12.vza.ch.zhaw.murbalio;
...
public class Reiseanbieter {
	...
	static void printStatistik() {
		...
		// System.out.println("Anzahl Verbindungen: "+g);
	}
	...
	static void printDirektflug(String von, String nach) {
		...
		Flughafen f1 = flughaefen.get(von);
		Flughafen f2 = flughaefen.get(nach);
		for (Verbindung v : f1.getVerbindungen()) {
			if ((v.getZiel().getName()) == f2.getName()) {
				System.out.print(v.getPreis());
			} else {
			}
		}
	}
	...
	static void printVerbindungen(String von, int stops) {
		...
		if (stops > 1) {
			stops--;			
			for (Verbindung v : f1.getVerbindungen()) {
				printVerbindungen(v.getZiel().getKuerzel(), stops);
