package uebung12.vza.ch.zhaw.heinini2;
...
public class Reiseanbieter {
	...
	static void printStatistik() {
		...
		int verbindungentotal=0;
		for (Flughafen i : flughaefen.values()) {
			verbindungentotal+= i.getVerbindungen().size();
		}
		System.out.println("Anzahl Verbindungen:"+ verbindungentotal);
	}

	static void printFluege(String von) {

		for (Flughafen i : flughaefen.values()) {
			if (i.getKuerzel().equals(von)) {
				System.out.println("Alle Flüge ab "+i.getName());
				for (Verbindung j : i.getVerbindungen()) {
					System.out.println(" -> "+j.getZiel().getName()+": "+j.getPreis());
				}
			}
		}
	}
	...
	static void printDirektflug(String von, String nach) {
		...
		boolean check = true;
		for (Verbindung i : flughaefen.get(von).getVerbindungen()) {
			if (i.getZiel().getKuerzel().equals(nach)) {
				System.out.println("Direktflug von "+flughaefen.get(von).getName()+" nach "+flughaefen.get(nach).getName()+" : "+i.getPreis());
				check= false;
			}
		}
		if (check) {
	...
	static void printVerbindungen(String von, int stops,double preis, String path) {
		...
		if (stops>=0) {
			...
			for (Verbindung v : f.getVerbindungen()) {
				...
				printVerbindungen(v.getZiel().getKuerzel(), stops,preis+=v.getPreis(),path);
				
			}
		} else {
			System.out.println(f.getName()+": "+preis+" Route: "+path);
