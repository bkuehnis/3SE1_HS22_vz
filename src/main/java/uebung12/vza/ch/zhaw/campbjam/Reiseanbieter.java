package uebung12.vza.ch.zhaw.campbjam;
...
public class Reiseanbieter {
	...
	public static void main(String[] args) {
		...
		while (!auswahl.equals("e")) {
			...
			if (auswahl.equals("s")) {
				...
				System.out.println("\nVerbindungen 2");
				printVerbindungen2(von, stops, 0);
				System.out.println("\nVerbindungen 3");
				printVerbindungen3(von, stops, 0, "");
			}
		}
	...
	static void printStatistik() {
		...
		for (Flughafen f : flughaefen.values()) {
			anzahlVerbindungen += f.getVerbindungen().size();
		}
	...
	static void printDirektflug(String von, String nach) {
		...
		fVon.getVerbindungen().stream().filter(x -> x.getZiel().getKuerzel().equals(nach))
