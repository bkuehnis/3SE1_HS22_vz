package uebung12.vzb.ch.zhaw.toddara1;
...
public class Reiseanbieter {
    ...
    static void printFluege(String von) {
        ...
        flughaefen.get(von).getVerbindungen().forEach(v -> {
            System.out.println(" -> " + v.getZiel().getName() + ": " + v.getPreis());
        });
    }

    static void printDirektflug(String von, String nach) {
        Optional<Verbindung> verbindung = flughaefen.get(von)
                .getVerbindungen()
                .stream()
                .filter(v -> v.getZiel().getKuerzel().equalsIgnoreCase(nach))
                .findFirst();
        double preis = 0;
        if(verbindung.isPresent()){
            preis = verbindung.get().getPreis();
        }
        System.out.println("Direktflug von " + flughaefen.get(von).getName()
                + " nach " + flughaefen.get(nach).getName() + ": " + (preis != 0.0 ? preis : ""));
    }
    ...
    static void printVerbindungen(String von, int stops, double kosten, String route) {
        ...
        for(Verbindung v : flughaefen.get(von).getVerbindungen()) {
            if(stops > 0){
                printVerbindungen(v.getZiel().getKuerzel(), (stops - 1), kosten + v.getPreis(), route);
            } else if (stops == 0) {
                System.out.println(v.getZiel().getName() + ": " + (kosten + v.getPreis()) + " Route: " + route + v.getZiel().getName());
            }
        }
