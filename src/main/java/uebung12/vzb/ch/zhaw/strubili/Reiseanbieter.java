package uebung12.vzb.ch.zhaw.strubili;
...
public class Reiseanbieter {
	...
	static void printStatistik() {
		...
		for (Flughafen x: flughaefen.values()) {
			counter += x.getVerbindungen().size();
		}
		System.out.println("Anzahl Vebindungen: "+counter);
	...
	static void printFluege(String von) {
		...
		for (Verbindung x: f.getVerbindungen()) {
			System.out.println("--> "+x.getZiel().getName()+": "+x.getPreis());
		}
	}
	
	static void printDirektflug(String von, String nach) {
		Flughafen v = flughaefen.get(von);
		Flughafen n = flughaefen.get(nach);
			System.out.print("Direktflug von "+v.getName()+" nach "+n.getName()+": ");
			v.getVerbindungen().stream()
			.filter(y -> y.getZiel().getKuerzel().equals(nach))
			.forEach(y -> System.out.println(y.getPreis()+" "));
		}
	
	//Ab hier komme ich nicht weiter. Ich verstehe das Vorgehen bei der Rekursion nicht ganz.
	static void printVerbindungen(String von, int stops) {
		Flughafen n = flughaefen.get(von);
		if (stops > 0) {
			for (Verbindung x: n.getVerbindungen()) {
				printVerbindungen(x.getZiel().getKuerzel(), stops-1);
			}
		} else {
