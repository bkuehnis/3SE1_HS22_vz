package vza.ch.zhaw.drmicfil.uebung1;


public class Rechner {

	public static void main(String[] args) {
		
		double fzA = 5; // Länge von deinem Auto (Meter)
		double fzB = 5; // Länge des anderen Autos (Meter)
		double vA = 50; // Deine Geschwindigkeit (km/h)
		double vB = 30; // Geschwindigkeit des anderen Autos(km/h)
		
		double Sicherheitsabstand1 = vA/2;
		double Sicherheitsabstand2 = vB/2;
		double RelativeStrecke = fzA+Sicherheitsabstand1+fzB+Sicherheitsabstand2;
		double RelativeGeschwindigkeit = vA-vB;
		double BenoetigteZeit = (RelativeStrecke*3.6)/RelativeGeschwindigkeit;
		double Ueberholweg = (BenoetigteZeit*vA)/3.6;
		
		
		System.out.println("Geschwindigkeit A: "+vA);
		System.out.println("Geschwindigkeit B: "+vB);
		System.out.println("Länge A: "+fzA);
		System.out.println("Länge B: "+fzB);
		System.out.println("Sicherheitsabstand 1: "+Sicherheitsabstand1);
		System.out.println("Sicherheitsabstand 2: "+Sicherheitsabstand2);
		System.out.println("Relative Strecke: "+RelativeStrecke);
		System.out.println("Relative Geschwindigkeit: "+RelativeGeschwindigkeit);
		System.out.println("Benoetigte Zeit: "+BenoetigteZeit);
		System.out.println("Ueberholweg: "+Ueberholweg);
		
	

	}

}
