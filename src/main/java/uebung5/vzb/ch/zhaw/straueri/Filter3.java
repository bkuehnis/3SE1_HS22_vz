package uebung5.vzb.ch.zhaw.straueri;
...
public class Filter3 {
	...
	public static void main(String[] args) {
		...
		for (int h = 10; h < (frame.length-10); h++) {
			for (int w = 10; w < frame[0].length-10; w++) {
				frame[h][w] = bild[h-10][w-10] ;
				
		
			}
		}
		for (int f = 0; (f < 10)||(f>frame.length-10); f++) {
			for (int P = 0; (P < 10)||(P>frame[0].length-10); P++) {
			frame[f][P] = frame[f][P]^0x6495ED ;
		}
		}
		// show result
