	...
	public static void main(String[] args) {
		...
		for (int h = 0; h < frame.length; h++) {
			for (int w = 0; w < frame[0].length; w++) {
				if (h<=frameWidth || h>=frame.length-frameWidth || w<=frameWidth || w>=frame[0].length-frameWidth){ //Bereich des Rahmens definieren
					frame[h][w] = blue;
				} else {
					frame[h][w] = bild[h-frameWidth][w-frameWidth]; //Bild
				}
			}
		}
