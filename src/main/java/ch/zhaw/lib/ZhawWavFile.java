...
public class ZhawWavFile implements AutoCloseable {
	...
	private static final String MSG_CANT_READ_FROM_WAVFILE = "Cannot read from WavFile instance";
	private static final String MSG_CANT_WRITE_TO_WAVFILE = "Cannot write to WavFile instance";
	...
	private static void readAndCheckWaveHeader(File file, ZhawWavFile wavFile) throws ZhawWavFileException, IOException {
		// Read the first 12 bytes of the file
		int bytesRead = wavFile.iStream.read(wavFile.buffer, 0, 12);
		if (bytesRead != 12)
			throw new ZhawWavFileException("Not enough wav file bytes for header");
		...
		if (riffChunkID != RIFF_CHUNK_ID)
			throw new ZhawWavFileException("Invalid Wav Header data, incorrect riff chunk ID");
		if (riffTypeID != RIFF_TYPE_ID)
			throw new ZhawWavFileException("Invalid Wav Header data, incorrect riff type ID");

		// Check that the file size matches the number of bytes listed in header
		if (file.length() != chunkSize + 8) {
			throw new ZhawWavFileException(
					"Header chunk size (" + chunkSize + ") does not match file size (" + file.length() + ")");
		}
	}
	
	private static void processFormatChunk(ZhawWavFile wavFile, long numChunkBytes)
			throws IOException, ZhawWavFileException {
		int bytesRead;
		// Read in the header info
		bytesRead = wavFile.iStream.read(wavFile.buffer, 0, 16);
		if (bytesRead != 16)
			throw new ZhawWavFileException("Could not read 16 bytes from wave header info");

		// Check this is uncompressed data
		int compressionCode = (int) getLE(wavFile.buffer, 0, 2);
		if (compressionCode != 1)
			throw new ZhawWavFileException("Compression Code " + compressionCode + " not supported");
		...
		if (wavFile.numChannels == 0)
			throw new ZhawWavFileException("Number of channels specified in header is equal to zero");
		if (wavFile.blockAlign == 0)
			throw new ZhawWavFileException("Block Align specified in header is equal to zero");
		if (wavFile.validBits < 2)
			throw new ZhawWavFileException("Valid Bits specified in header is less than 2");
		...
		numChunkBytes -= 16;
		if (numChunkBytes > 0) {
			long bytesSkipped = wavFile.iStream.skip(numChunkBytes);
			if (bytesSkipped != numChunkBytes) {
				throw new ZhawWavFileException("Format Chunk: Could not skip over " + numChunkBytes + ", only "
						+ bytesSkipped + " Bytes were skipped");
			}
		}
	}

	private static void processDataChunk(ZhawWavFile wavFile, long chunkSize) throws ZhawWavFileException {
		// Check that the chunkSize (wav data length) is a multiple of the
		// block align (bytes per frame)
		if (chunkSize % wavFile.blockAlign != 0)
			throw new ZhawWavFileException("Data Chunk size is not multiple of Block Align");

		// Calculate the number of frames
		wavFile.numFrames = chunkSize / wavFile.blockAlign;
	}
	
	private static void processUnknownChunk(ZhawWavFile wavFile, long numChunkBytes)
			throws IOException, ZhawWavFileException {
		// If an unknown chunk ID is found, just skip over the chunk data
		long bytesSkipped = wavFile.iStream.skip(numChunkBytes);
		if (bytesSkipped != numChunkBytes) {
			throw new ZhawWavFileException("Unknown Chunk: Could not skip over " + numChunkBytes + ", only "
					+ bytesSkipped + " Bytes were skipped");
		}
	}

	/// Calculate the scaling factor for converting to a normalized double
	private static void calculateOffsetAndScale(ZhawWavFile wavFile, IOState iostate) throws ZhawWavFileException {
		if (iostate == IOState.CLOSED) {
			throw new ZhawWavFileException("Cannot calculate Offset and Scale for iostate CLOSED");
		}
		...
		if (wavFile.validBits > 8) {
			...
			} else if (iostate == IOState.READING) {
		...
		wavFile.ioState = iostate;
	}

	public static ZhawWavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate)
			throws IOException, ZhawWavFileException {
		...
		if (numChannels < 1 || numChannels > 65535)
			throw new ZhawWavFileException("Illegal number of channels, valid range 1 to 65536");
		if (numFrames < 0)
			throw new ZhawWavFileException("Number of frames must be positive");
		if (validBits < 2 || validBits > 65535)
			throw new ZhawWavFileException("Illegal number of valid bits, valid range 2 to 65536");
		if (sampleRate < 0)
			throw new ZhawWavFileException("Sample rate must be positive");
		...
		calculateOffsetAndScale(wavFile, IOState.WRITING);
	...
	public static ZhawWavFile openWavFile(File file) throws IOException, ZhawWavFileException {
		...
		readAndCheckWaveHeader(file, wavFile);
		...
		while (true) {
			...
			if (bytesRead == -1)
				throw new ZhawWavFileException("Reached end of file without finding format chunk");
			if (bytesRead != 8)
				throw new ZhawWavFileException("Could not read chunk header");
			...
			if (chunkID == FMT_CHUNK_ID) {
				...
				processFormatChunk(wavFile, numChunkBytes);
				...
				if (!foundFormat)
					throw new ZhawWavFileException("Data chunk found before Format chunk");

				processDataChunk(wavFile, chunkSize);
				break;
			} else {
				processUnknownChunk(wavFile, numChunkBytes);
			}
		}

		calculateOffsetAndScale(wavFile, IOState.READING);
	...
	private long readSample() throws IOException, ZhawWavFileException {
		...
		for (int b = 0; b < bytesPerSample; b++) {
			if (bufferPointer == bytesRead) {
				...
				if (read == -1)
					throw new ZhawWavFileException("Not enough data available");
				bytesRead = read;
				bufferPointer = 0;
			}
	...
	public int readFrames(int[] sampleBuffer, int offset, int numFramesToRead)
		...
		if (ioState != IOState.READING)
			throw new IOException(MSG_CANT_READ_FROM_WAVFILE);
	...
	public int writeFrames(int[] sampleBuffer, int offset, int numFramesToWrite) throws IOException {
		if (ioState != IOState.WRITING)
			throw new IOException(MSG_CANT_WRITE_TO_WAVFILE);
	...
	public static void main(String[] args) {
		...
		try {
			for (String filename : args) {
				...
				ZhawWavFile writeWavFile = newWavFile(new File("out.wav"), numChannels, numFrames, validBits,
						sampleRate);
